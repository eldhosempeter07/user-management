...
import { add, del, get, getById, update } from "../../Helpers/api_helpers";
import {
  AddUserProps,
  deleteUserProps,
  getRequest,
...
interface AddUserAction extends Action, AddUserProps {
  type: "ADD_USER";
}
interface GetUsersAction extends Action, getRequest {
  type: "GET_USER";
}
interface GetUserByIdAction extends Action, GetUserByIdProps {
  type: "GET_USER_BY_ID";
}
interface UpdateAction extends Action, updateUserProps {
  type: "UPDATE_USER";
}
interface DeleteAction extends Action, deleteUserProps {
  type: "DELETE_USER";
}
...
function* getUserbyId(res: getUser): Generator {
  try {
    ...
    const response = yield call(getById, `/${res.data.id}`);
    if (response) {
